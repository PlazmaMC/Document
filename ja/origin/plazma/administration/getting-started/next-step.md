...
[詳細については、`リファレンス > 引数とプロパティ`を参照してください。](../reference/arguments.md#safemode)
...
Plazmaには多くの最適化パッチが適用されています。 また、Plazmaが初めて起動されると、自動的に
構成が最適化されるため、[はじめに](./README.md) の説明書に従った場合、追加の最適化作業は必要ありません。

ただし、多くのプレイヤーが接続したり、ワールドのサイズが膨大な場合、
以下の説明書に従って追加の最適化作業を行うことができます。
