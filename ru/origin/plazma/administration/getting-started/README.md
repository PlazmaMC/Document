...
Если выводится как указано ниже, перейдите к [Шагу 2](#id-2).
...
Если выводится не так, как указано выше, или выводится как указано ниже, это означает, что JRE отсутствует или слишком стар, поэтому необходимо выполнить [Шаг 1.2](#id-1.2).
...
В данном руководстве используется Azul Zulu в качестве одного из [типов JRE](#user-content-fn-5)[^5].

После завершения установки выполните [Шаг 1.1](#id-1.1) снова, чтобы убедиться, что установка завершена правильно.
...
Вы можете установить JRE, выполнив следующую команду.
...
Следующая информация предназначена для разработчиков или тех, кто интересуется особенностями различных форм.\
Если вы обычный пользователь, можете пропустить к [Шагу 3](#id-3) без проблем.
...
- **Тип маппинга**\
  ...
  - **Reobf**\
    Reobfuscated (재난독화), также известный как Spigot маппинг, используется в большинстве плагинов NMS.\
    С 1.20.5 его использование будет прекращено.
  - **Mojmap**\
    Mojang-маппинг, это маппинг для ванильного Minecraft.
- **Менеджер версий**\
...
Для Windows, так как брандмауэр разрешен на [Шаге 3](#id-3), вам нужно только настроить переадресацию портов.

{% hint style="info" %}

**Это руководство предполагает, что вы используете операционную систему Windows и маршрутизатор, который поддерживает [UPnP](#user-content-fn-12)[^12].**
