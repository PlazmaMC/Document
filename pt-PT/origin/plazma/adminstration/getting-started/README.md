...
{% tab title="macOS" %}
Faça o download do assistente de instalação do **JDK 21** em formato `.dmg` do [Azul Zulu](https://www.azul.com/downloads/?version=java-21-lts\&os=macos\&architecture=x86-64-bit\&package=jdk#zulu) e execute para instalar o JRE.
{% endtab %}

{% tab title="Debian/Ubuntu" %}
Primeiro, execute o seguinte comando no terminal para adicionar o repositório Azul Zulu ao APT.
...
### Na maioria dos casos, use o `Reobf Paperclip`.
...
Agora, execute o script de inicialização. No caso do Windows, <mark style="background-color:orange;">você deve selecionar</mark> <mark style="background-color:orange;">**Permitir**</mark><mark style="background-color:orange;"> na janela de seleção de permissão do firewall</mark>.
...
### Este guia foi escrito considerando o sistema operacional Windows e um roteador que suporte [**UPnP**](#user-content-fn-12)[^12].
...
### Para sistemas baseados em UNIX, como Linux ou macOS, os métodos de configuração do firewall variam para cada serviço, portanto, é necessário pesquisar as informações específicas.
...
Se não for necessário encaminhamento de porta, ou se já tiver sido bem-sucedido, agora poderá acessar o servidor.

O endereço usado para acessar o servidor pode ser verificado [aqui](https://ip.pe.kr/).
{% endtab %}

{% tab title="Tentativa de encaminhamento de porta UPnP" %}
...
A utilização do Ngrok é útil para testes temporários, jogar com amigos ou participar.
...
Se estiver tentando acessar o servidor localmente, execute `cmd /k ipconfig` na janela de execução para obter o `Endereço IPv4` exibido.
