...
### تعديل خصائص النظام يمكن أن يؤدي إلى تغيير في عمل Plazma وJVM، وقد يؤثر بشكل كبير على اللعبة!

إذا لم تكن متأكدًا من دور كل خاصية نظامية، **فلا تستخدمها أبدًا!**
{% endhint %}
...
### تحتوي منصات الخادم من سلسلة Paperweight على نقطة في اسم الخاصية لتمييز خصائص النظام لكل منصة.

في بعض الطرفيات مثل Windows Powershell، قد لا تسمح ببعض هذه السمات، لذا يجب إضافة `"` في كلا الطرفين للسمة.]\(#user-content-fn-4)[^4]
{% endhint %}
...
### نظرًا لعدم استقرار أمر `/reload`، فإن جميع المشاكل التي تظهر بعد استخدام `/reload` تكون مسؤولية المستخدم نفسه.
...
### هذا قد يسمح لـ JVM بالوصول إلى رمز غير موجود!
...
### سيتم نقل هذه الخاصية كمعلمة بدء بعد الإصدار 1.20.5.
...
### تم إيقاف استخدام هذه الوظيفة بعد 1.19.4
